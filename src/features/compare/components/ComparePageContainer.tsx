...
const initialForm = (): CompareBuildForm => ({
  roleId: 'dps', characterId: '', slot: 'cartridge', mainStatKey: STAT_KEYS[0], mainStatValue: '', subStats: Array.from({ length: 3 }, () => ({ key: STAT_KEYS[0], value: '' })),
});
...
export function ComparePageContainer() {
  ...
  const compareResult = useMemo(() => (errorsA.length === 0 && errorsB.length === 0 ? calculateCompareResult(formA, formB) : null), [errorsA.length, errorsB.length, formA, formB]);
