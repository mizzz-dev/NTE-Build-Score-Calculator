...
type Props = { status: ScoreOcrStatus; error: string | null; rawText: string; candidate: ScoreOcrCandidate | null; statKeyOptions: StatKey[]; panelTitle?: string; onRawTextChange: (value: string) => void; onSelectImage: (file: File | null) => void; onUpdateCandidate: (next: ScoreOcrCandidate) => void; onApplyCandidate: () => void };
...
function buildNextActionHints(status: ScoreOcrStatus, hasCandidate: boolean) {
  if (status === 'error') {
    return [
      '同じ画像で再試行（ステータス部分が切れていないか確認）',
      '別画像で再撮影して再試行（明るさ・傾き・ブレを見直す）',
  ...
  return [
    '画像を選択してOCRを開始',
    '読み取り失敗や低品質疑い時は、再撮影で再試行または手動入力へ切替',
