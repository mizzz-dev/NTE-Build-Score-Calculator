...
Issue #133 で `image_quality_low` 起因の事前撮影ガイドと再撮影導線を改善します。

目的:
- OCR実行前に撮影品質を上げやすい短いガイドを表示する。
- 文字が読める明るさ、傾き・ブレ回避、対象ステータス部分の欠け防止を案内する。
- 個人情報やアカウント情報が写り込まないよう注意を含める。
- OCR失敗・低品質疑い時に、再撮影と手動入力のどちらを選べるか分かる案内にする。
- OCRアルゴリズム、OCR信頼度計算、保存payload仕様は変更しない。

## 5. 進行中 / 次フェーズ

- 進行中: Issue #133（`image_quality_low` 起因の事前撮影ガイド・再撮影導線改善）
- 次候補:
