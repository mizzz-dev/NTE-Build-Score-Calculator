...
最終更新: 2026-05-20（Issue #169 deploy前後条件分離反映）
...
実施内容:
- `docs/runbooks/official-release-approval-and-evidence-submission-template.md`（Issue #169反映版） を追加。
...
Issue #169 で承認・証跡提出テンプレートのdeploy前後条件分離を実施しました。
...
実施結果:
- 公開実行前Go判定条件からdeploy実行ログURLを除外。
- deploy実行ログURLをdeploy後確認項目へ分離。
- 最終判定チェックを「公開実行前Go判定チェック」「公開実行後確認チェック」に分割。
- deploy前条件を「人間最終Go承認 / 本番公開URL一致証跡 / 検索公開設定証跡 / rollback参照コミット・方針確認」に整理。
- 保存可/保存不可情報方針は維持。

## 5. 進行中 / 次フェーズ

- 完了: Issue #169（承認・証跡提出テンプレートのdeploy前後条件分離）
- 次候補:
  1. 人間最終Go承認の記録（承認者・時刻・URL、公開実行前Go判定）
  2. 実環境証跡の保存
