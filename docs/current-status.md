...
正式リリース準備として、公開前チェックリスト、リリースノート案、本番公開前のGo / No-Go判定レポートをRepositoryへ保存済みです。ただし、PR #153のレビューで主要機能ページのcanonical方針とGo判定レポート記載の不整合が指摘されたため、次にIssue #154で修正します。
...
Issue #154 で主要機能ページのcanonical方針を実装とレポートで整合させます。
...
- 進行中: Issue #154（主要機能ページのcanonical方針整合）
- 次候補:
