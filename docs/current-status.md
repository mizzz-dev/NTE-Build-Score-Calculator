...
## 3. 直近完了タスク（Issue #169 / PR #170）

Issue #169 / PR #170 で承認・証跡提出テンプレートのdeploy前後条件分離を実施しました。

実施結果:
- `docs/runbooks/official-release-approval-and-evidence-submission-template.md` を修正。
...
- 公開実行前Go判定: **No-Go継続**
- 理由: 人間最終Go承認・本番公開URL一致証跡・検索公開設定証跡が未確認、rollbackは参照コミット/方針のみ確認で人間判定記録が未充足。
- 運用維持: deploy実行ログURLは公開実行後確認項目として扱い、公開実行前Go判定条件に含めない。
- 保存可/保存不可情報方針は維持。
- 作業ログ: `docs/logs/2026-05-20-issue-169-deploy-gate-separation.md`
- AIプロンプトログ: `docs/ai-prompts/2026-05-20-issue-169-deploy-gate-separation.md`

## 4. 現在の次作業（Issue #171）

Issue #171 で人間最終Go承認と本番公開条件証跡の提出状況を記録し、公開実行前Go判定を更新します。

目的:
- 修正済みテンプレートに沿って、人間最終Go承認を記録する。
- 本番公開URL一致証跡を記録する。
- 検索公開設定証跡を記録する。
- rollback参照コミット / rollback方針を記録する。
- 公開実行前Go判定を `Go` または `No-Go継続` として根拠付きで保存する。
- deploy実行ログURLは公開実行後確認項目として扱い、公開実行前Go判定条件には含めない。

## 5. 進行中 / 次フェーズ

- 進行中: Issue #171（人間最終Go承認と本番公開条件証跡の不足項目解消待ち）
- 次候補:
  1. 公開実行前Go判定
  2. 条件充足時の公開実行Issue作成
