...
- #193 / PR #202: Gate A人間確認・Deploy-Go判定Issue Formの追加（closed / merged）
...
1. 完了状態同期PRマージ後の`main` HEAD確定（P1）
2. 最終deploy対象SHAに対する`release-preflight`実行・成功確認（P1）
3. Gate A Deploy-Go判定Issueの起票・人間入力（P1 / 人間対応）
4. Gate AがGoの場合の初回本番deploy（P1）
5. Gate Bの本番実測確認（P1）
6. 公開継続またはrollback判断（P1）
7. post-release実測結果への更新（P1）
8. 商用化前の専門確認（名称・素材・免責文言・収益化導線）（P1）
9. rollback運用監査（P2）

## 3. 優先順位

- P1: 最終deploy対象SHAに対するrelease-preflight成功証跡を確定する。
- P1: Gate A Issue Formへ人間確認を漏れなく記録する。
...
Issue Formから生成したIssueを、Gate A人間確認のSource of Truthとする。
...
- Gate A人間確認・Deploy-Go判定Issue Form。
...
- Deploy-Goの人間判定はPRレビュー承認とは別のリリース判断として維持する。
- PR headのCI成功をdeploy対象SHAのrelease-preflight成功として推測補完しない。
...
- 本番deploy、Vercel / Netlify設定変更、secret操作は人間の明示判断なしに行わない。
