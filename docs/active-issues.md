...
- #99: `/score` OCR品質評価 第3サイクル実測ログ作成（完了）
...
## 2. 次にやるべきIssue候補
1. OCR KPI全項目の連続達成確認（次サイクル、展開判定条件）
2. 低スペック端末カテゴリp95短縮の継続検証（4.4秒→4.0秒近傍）
3. 低解像度・ぼやけ・韓国語カテゴリの品質底上げIssue起票
4. `/card` `/compare` OCR展開の要件再評価（連続達成確認後）

## 3. 優先順位（暫定）
- P1: OCR KPI全項目の連続達成確認（展開判定ゲート）
- P1: 低スペック端末カテゴリp95短縮の継続
- P2: 低解像度/ぼやけ/韓国語カテゴリの品質底上げ
- P3: `/card` `/compare` OCR展開検討

## 4. Blocker
- 第3サイクルで全KPI達成（p95=3.9秒）を確認したが、展開判定基準は「連続2サイクル達成」のため、次サイクル確認が必要。
- 低スペック端末カテゴリは改善（p95=4.4秒）したものの、カテゴリ単体の短縮余地が残る。
